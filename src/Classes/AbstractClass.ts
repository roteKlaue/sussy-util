...
export default class AbstractClass<T extends object> {
  public constructor(Constructor: Constructor<T>) {
    const isClass = (arg: unknown): arg is Constructor<T> => {
    ...
    if (!isClass(Constructor))
      throw new TypeError('Constructor must be the constructor of a class.');
    if (this.constructor === Constructor) {
      ...
      throw new AbstractClassInstanceCallError(
        "You can't instantiate a new instance of a abstract class.",
      );
    }
