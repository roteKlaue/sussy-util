...
class BetterMath {
  ...
  public median(values: number[]): number {
    ...
    if (length % 2 === 0) {
      return (
        (ArrayUtils.quickSelect(values, middle) + ArrayUtils.quickSelect(values, middle + 1)) / 2
      );
    }

    return ArrayUtils.quickSelect(values, middle + 1);
  }
   ...
   * @returns {number} The factorial of the number.
   */
  public factorialWithMemoization(
    num: number,
