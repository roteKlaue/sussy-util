...
export default class StringUtil {
  private static dedupeChars(s: string): string {
    return Array.from(new Set(s.split(''))).join('');
  }

  private static safeCharset(charset?: string) {
  ...
  public static randomCharacter(charset?: string): string {
    ...
    const idx = Random.randomInt(0, cs.length);
  ...
  public static toCharCode(str: string): string {
    return str
      .split('')
      .map((val: string) => val.charCodeAt(0))
      .join('');
  }
  ...
  public static generatePassword(length: number, characters?: string): string {
    ...
    if (uniqueChars.length < 8) throw new Error('Not enough characters to generate password');
...
export const reverse = (s: string) => StringUtil.reverse(s);
export const isEmpty = (s?: string | null) => StringUtil.isEmpty(s);
export const randomString = (len: number, cs?: string) => StringUtil.randomString(len, cs);
export const slugify = (s: string) => StringUtil.slugify(s);
