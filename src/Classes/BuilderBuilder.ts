...
class GenericBuilder<T extends {}> implements Builder<T> {
  public constructorParams: { order: Array<string | string[]>; attributes: MutableObject<any> } = {
    order: [],
    attributes: {},
  };
  ...
  constructor(
    ...
    Type === 'objectPropertyReading'
      ? this.createSettersProperties()
      : this.createSettersSignature();
  ...
  private createSettersSignature() {
    ...
    this.constructorParams.order = this.interpretConstructor(
      parameterNames[0].substring(1, parameterNames[0].length - 1),
    );
    ...
    for (let i = 0; i < this.constructorParams.order.length; i++) {
      if (Array.isArray(this.constructorParams.order[i])) {
        ...
        this.constructorParams.attributes[`param${i}`].length =
          this.constructorParams.order[i].length;
  ...
  build(): T {
    ...
    return new this.ClassType(
      ...this.constructorParams.order.map((e, i) => {
        if (Array.isArray(e)) {
 ...
 */
export default class BuilderBuilder<T extends {}> implements Builder<
  Constructor<GenericBuilder<T>>
> {
  /**
  ...
  constructor(
    readonly ClassType: Constructor<T>,
    readonly CreationType: 'constructorReading' | 'objectPropertyReading' = 'objectPropertyReading',
  ) {}
