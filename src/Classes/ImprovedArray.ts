...
export default class ImprovedArray<T> extends Array<T> {
   ...
   */
  public rejected(
    predicate: (val: T, ind: number, arr: T[]) => boolean,
    thisArg?: unknown,
  ): ImprovedArray<T> {
    return new ImprovedArray<T>(
      ...this.filter((val, ind, arr) => !predicate(val, ind, arr), thisArg),
    );
  }
  ...
  public each(callbackfn: (value: T, index: number, array: T[]) => void, thisArg?: this): void {
  ...
  public deepClone(): ImprovedArray<T> {
    return new ImprovedArray<T>(...this.map(deepClone));
  }
}
