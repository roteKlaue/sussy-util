...
describe('Optional', () => {
	...
	it('should create an Optional with a value when ofNullable is called with a non-null value', () => {
		const optional = Optional.ofNullable(42);
		expect(optional.isPresent()).toBe(true);
		expect(optional.get()).toBe(42);
	});

	it('should create an empty Optional when ofNullable is called with null', () => {
		const optional = Optional.ofNullable(null);
		expect(optional.isPresent()).toBe(false);
		expect(optional.get()).toBeUndefined();
	});

	it('should create an empty Optional when ofNullable is called with undefined', () => {
	...
	it('should flatMap the value to a new Optional when flatMap is called and the value is present', () => {
		const optional = Optional.of(42);
		const flatMappedOptional = optional.flatMap(x => Optional.of(x * 2));
		expect(flatMappedOptional.isPresent()).toBe(true);
		expect(flatMappedOptional.get()).toBe(84);
	});

	it('should return an empty Optional when flatMap is called and the value is not present', () => {
		const optional = Optional.empty<number>();
		const flatMappedOptional = optional.flatMap(x => Optional.of(x * 2));
		expect(flatMappedOptional.isPresent()).toBe(false);
		expect(flatMappedOptional.get()).toBeUndefined();
	});
